...
static cli_command_result parse_get(const char *argument, size_t argument_length)
{
    cli_command_result result = invalid_result();
    const char *id_end = argument;
    const char *trailing;
...
cli_command_result cli_parse_line(const char *line)
{
    ...
    if (starts_with_command(line, line_length, "get"))
    {
        argument = line + strlen("get");
        while ((size_t)(argument - line) < line_length &&
               (*argument == ' ' || *argument == '\t'))
        {
            argument++;
        ...
        return parse_get(argument, line_length - (size_t)(argument - line));
    }
